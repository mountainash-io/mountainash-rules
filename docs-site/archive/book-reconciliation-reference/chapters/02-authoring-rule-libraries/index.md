...
!!! note "Unreconciled reference chapter"
    This page retains an earlier organization and source basis. For current explanations of dimension configuration and matching semantics, read [Chapter 2](../02-shared-rule-model/index.md) and [Chapter 3](../03-matching-concepts/index.md).

The reconciled opening used a table and dimension metadata with both engines. This reference chapter develops the metadata in more detail: how does the engine know that `region` should use exact equality, while `order_total_min` and `order_total_max` describe a range? `Dimension` and `DimensionsMetadata` provide that configuration.
...
Every compiled strategy produces a ternary value, `1` (TRUE), `0` (UNKNOWN) or `-1` (FALSE), for each rule row against the current context. The [accepted book's ternary explanation](https://docs.mountainash.io/mountainash-rules/dev/chapters/01-foundation-concepts/#ternary-logic) provides the detailed foundation while the new matching chapter is being reconciled. What differs between strategies is how that value is derived: by equality, by an ordered comparison, by a string operation, or by set membership.
