...
!!! note "Unreconciled reference chapter"
    This page retains the donor's organization and older source basis. It supplies reference material for the approved Chapter 9, not its completed replacement. See the [book contents](../index.md) for the chapter-by-chapter review status.

The usage references cover rule tables authored with [`Dimension`](../02-authoring-rule-libraries/index.md) metadata, evaluated with [`ExpressionRulesEngine.evaluate()`](../03-evaluating-decisions/index.md), and selected with a [`HitPolicy`](../03-evaluating-decisions/index.md). The approved [batch chapter](../06-batch-evaluation/index.md) extends evaluation to many contexts. This reference traces how those operations were implemented at the donor's source revision.

The walkthrough follows five modules: `src/mountainash_rules/core/compiler.py`, `src/mountainash_rules/core/context.py`, `src/mountainash_rules/core/constants.py`, `src/mountainash_rules/core/hit_policy.py`, and `src/mountainash_rules/engines/filter/engine.py`, at commit [`94659bb`](https://github.com/mountainash-io/mountainash-rules/blob/94659bb0c096485c87d329f09e944577427f129f). The accepted book provides detailed [expression and relation foundations](https://docs.mountainash.io/mountainash-rules/dev/chapters/01-foundation-concepts/#mountainash-expressions) and [ternary logic](https://docs.mountainash.io/mountainash-rules/dev/chapters/01-foundation-concepts/#ternary-logic) while those new shared-foundation chapters are reconciled.
