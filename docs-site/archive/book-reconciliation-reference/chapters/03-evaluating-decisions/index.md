...
!!! note "Unreconciled reference chapter"
    This page retains the donor's organization and older source basis. It supplies reference material for the approved Chapters 4 and 5, not their completed replacements. See the [book contents](../index.md) for the chapter-by-chapter review status.
...
A `RuleResult` contains the rules that survived every active dimension, ranked and possibly reduced by a hit policy, with observability for explaining surviving rules. `engine.explain()` and `ExplainResult` provide the separate unfiltered explanation surface. The approved [batch chapter](../06-batch-evaluation/index.md) develops these ideas for many contexts at once. The [expression-internals reference](../06-expression-execution-internals/index.md) describes the donor's implementation walkthrough and still awaits reconciliation against current source.
