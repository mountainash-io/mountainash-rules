...
`index.apply()` routes one context and then evaluates it against the selected lattice. `index.apply_batch()` normalizes context-key fields, groups rows by normalized key, routes each group, evaluates each group with the lattice's filter engine, and merges the survivors while preserving a global context ID. This gives single and batch serving the same partition-routing semantics.

`apply_batch()` is a routing wrapper around per-partition `evaluate_batch()` calls, not a drop-in replacement for calling `ExpressionRulesEngine.evaluate_batch()` directly. In the partitioned path, it synthesizes global positional context IDs when no truthy `context_id_field` is supplied; a caller-supplied ID is validated for uniqueness within each partition's own batch, not globally across partitions. The merged `BatchRuleResult` carries only the concatenated survivors, the constraint dimensions, and the context ID field — it does not re-sort the merged rows by context ID and rank, and it does not forward per-partition selection metadata (such as truncation) from the underlying `evaluate_batch()` calls. These caveats concern the partitioned merge path. With no context-key dimensions, the index directly returns its sole lattice's filter-engine batch result. Do not infer full direct-filter ordering, re-selection, or empty-input guarantees for the partitioned wrapper.
