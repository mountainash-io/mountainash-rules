...
This taxonomy organizes the 130 mountainash-rules concepts into 9 categories aligned with the package's
architecture layers. Updated 2026-09-02 (source `7d0e3dc`) to add two categories — Hit Policies and Batch
Evaluation — for capabilities added since the original 90-concept graph (`41d5846`, 2026-06-03).
...
The dimension metadata layer: the MatchStrategy enum with its 12 strategy types (EXACT, EXACT_KEY, NOT_EQUAL,
RANGE, GREATER_THAN, LESS_THAN, PREFIX, SUFFIX, CONTAINS, REGEX, CONTEXT_REGEX, SET_MEMBERSHIP, SET_EXCLUSION —
note REGEX split into per-row REGEX and metadata-literal CONTEXT_REGEX), the DimensionRole enum (CONSTRAINT vs
